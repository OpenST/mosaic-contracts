...
//
// ----------------------------------------------------------------------------
// Test: ProgressOutboxRevocation.js
//
...
const messageBusUtilsKlass = require('./messagebus_utils'),
	...
	ProgressOutboxRevocation = function(){};
...
let intentHash,
	...
	progressOutboxRevocation = async() => {
	...
	contract('MessageBus',  async (accounts) => {
		...
		describe('progressOutboxRevocation', async () => {
			...
			it('should fail when message status is undeclared ', async () => {
				...
				messageStatus = 0;
				await progressOutboxRevocation();
