...
contract('GatewayBase.getInboxActiveProcess()', (accounts) => {
  let gatewayBase;
  let messageHash;
  let accountAddress;
  ...
  beforeEach(async () => {
    ...
    await gatewayBase.setInboxProcess(accountAddress, messageHash);
  });
  ...
  it(
    'should return zero message hash and undeclared message status when the '
      + 'account address does not have active inbox process',
    async () => {
      await gatewayBase.setInboxStatus(
        messageHash,
        MessageStatusEnum.Declared,
      );
      ...
      const result = await gatewayBase.getInboxActiveProcess(accountAddress);
