...
contract('MerklePatriciaProof', function(accounts) {
    ...
    describe('Test Cases for Account proof verification', async () => {
        ...
        it('Should fail when encoded path is empty', async () => {
            let proofStatus = await merkleMock.verifyAccount.call(proofJson.account.sha3EncodedAccount, "", proofJson.account.rlpParentNodes, proofJson.account.stateRoot);
            assert.equal(proofStatus,false);
        })
    ...
    describe('Test Cases for Storage proof', async () => {
        ...
        it('Should pass for valid variable storage',async () =>{
            ...
            assert.equal(proofStatus[0],true);
        });
        ...
        it('Should fail for variable storage when encoded value is incorrect',async () =>{
            ...
            assert.equal(proofStatus[0],false);
        })
        ...
        it('Should fail for variable storage when encoded path is empty',async () =>{
            let proofStatus = await merkleMock.verifyStorage.call(proofJson.storage.variable.sha3ValueAtStorage, "", proofJson.storage.variable.rlpParentNodes, proofJson.storage.variable.storageRoot);
            assert.equal(proofStatus[0],false);
        })
