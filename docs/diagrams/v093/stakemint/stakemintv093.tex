...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   \begin{umlfragment}[type=alt, label=redeem, name=policy, inner xsep=2]
  ...
  \umlfpart[process]
    ...
    \begin{umlcall}[dt=65, op={: processRedeeming(redemptionIntentHash, unlockSecret)}]{Redeemer}{OpenSTUtility}
...
\end{umlfragment}
    ...
    \begin{umlfragment}[type=alt, label=unstake, name=policy, inner xsep=2]
	...
	\begin{umlcall}[dt= 35, op={: processUnstaking(redemptionIntentHash, unlockSecret)}]{OstDotCom}{OpenSTValue}
...
\end{umlcall}
  ...
  \umlfpart[revert]
   ...
   \begin{umlcall}[type=return, dt= 10, op={emit RevertedUnstake(uuid, redemptionIntentHash, redeemer, beneficiary, amountST)}]{OpenSTValue}{OstDotCom}
  	 \end{umlcall}
    \begin{umlcall}[type=return, dt= 10, op={emit RevertedUnstake}]{OpenSTValue}{Beneficiary}
  	 \end{umlcall} 	 
     %\end{umlcall} 
  \end{umlcall}
  \end{umlfragment}
