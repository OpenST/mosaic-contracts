...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   %%%
    ...
    \begin{umlcall}[dt=10, op={: redeem(uuid, amountUT, beneficiary, hashLock)}]{Redeemer}{OpenSTUtility}
    % transferfrom is called from openstutility on branded token
   \begin{umlcall}[dt =12, fill=green!20, op={: transferFrom(redeemer, OpenSTUtility, amountUT)}]{OpenSTUtility}{Branded Token}
   ...
   \begin{umlcall}[dt =17, fill=green!25,  type=return, op={<<UT(amountUT)>>}]{Branded Token}{OpenSTUtility}
   % event is emitted with redemptionintentHash which the beneficiary can listen to
    \begin{umlcall}[dt= 19, type=return, op={emit RedemptionIntentDeclared(redemptionIntentHash, brandedToken, beneficiary, amountUT, unlockHeight)}]{OpenSTUtility}{Beneficiary}
  \end{umlcall}
  \end{umlcall}
   \end{umlcall}   
    \end{umlcall}
    ...
    \begin{umlcall}[op={: confirmRedemptionIntent(uuid, redeemer, redeemerNonce, beneficiary, amountUT, unlockHeight, hashLock)}]{Beneficiary}{OpenSTValue}
	...
	\end{umlcallself}     
     ...
     \begin{umlcall}[op={: RedemptionIntentConfirmed(redemptionIntentHash, redeemer, beneificary, amountST, amountUT, expirationHeight )}]{OpenSTValue}{Beneficiary}
  \end{umlcall}
\end{umlcall}
