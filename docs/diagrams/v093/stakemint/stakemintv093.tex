  ...
  \begin{umlseqdiag}
   ...
   % \end{umlcall}
    ...
    \begin{umlfragment}[type=loop, name=mosaic]
      ...
      \begin{umlcall}[dt=205, op={new block(blockHeight, stateRoot)}]{Value Chain}{OstDotCom}
    ...
    \begin{umlcall}[dt=15, op={: proveOpenST(blockHeight, rlpEncodedAccount, rlpParentNodes)}, return={emit OpenSTProven(blockHeight, storageRoot, hashedAccount)}]{Facilitator}{CoreVC}
     ...
     %\end{umlcall}
      ...
      \begin{umlcallself}[op={require(verify(hashedAccount, encodedOpenSTRemotePath, rlpParentNodes, stateRoot))}]{CoreVC}
     \end{umlcallself}
      % OpenSTUtility store valid StakingIntentHash
      \begin{umlcallself}[op={store valid storageRoot}]{CoreVC}
      \end{umlcallself}
    \end{umlcall}
