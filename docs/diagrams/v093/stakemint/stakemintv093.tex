...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   % \end{umlcall}
    ...
    \begin{umlfragment}[type=loop, name=mosaic]
      ...
      \begin{umlcall}[dt=193, op={new block(blockHeight, stateRoot)}]{Utility Chain}{Foundation}
        ...
        %\begin{umlcall}[op={: report(UC, blockHeight, stateRoot)}]{Foundation}{RegistrarVC}
          \begin{umlcall}[op={: commitStateRoot(blockHeight, stateRoot)}]{Foundation}{CoreUC}
      ...
      \begin{umlcall}[dt=205, op={new block(blockHeight, stateRoot)}]{Value Chain}{Foundation}
        %\begin{umlcall}[op={: report(VC, blockHeight, stateRoot}]{Foundation}{RegistrarUC}
          ...
          \begin{umlcall}[op={: commitStateRoot(blockHeight, stateRoot)}]{Foundation}{CoreVC}
    ...
    \begin{umlcall}[dt=15, op={: proveOpenST(blockHeight, rlpEncodedAccount, rlpParentNodes)}, return={emit OpenSTProven(blockHeight, storageRoot, hashedAccount)}]{Facilitator}{OpenSTUtility}
     ...
     %\end{umlcall}
      ...
      %\begin{umlcallself}[op={require(MerklePatriciaProof.verify(hashedAccount, encodedOpenSTRemotePath, rlpParentNodes, stateRoot))}]{CoreVC}
     %\end{umlcallself}
      % OpenSTUtility store valid StakingIntentHash
      \begin{umlcallself}[op={store valid storageRoot}]{OpenSTUtility}
      \end{umlcallself}
    \end{umlcall}
