...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   %%%
    ...
    \begin{umlcall}[dt=10, op={: redeem(uuid, amountUT, nonce, beneficiary, hashLock)}]{OstDotCom}{OpenSTUtility}
  ...
  %\end{umlcall}
    ...
    \begin{umlcall}[dt= 15, op={: confirmRedemptionIntent(redeemer, redeemerNonce, beneficiary, amountUT, redemptionUnlockHeight, blockHeight, hashLock, rlpParentNodes)}]{OstDotCom}{OpenSTValue}
  ...
  \end{umlcall}
    ...
    \begin{umlcall}[op={: processRedeeming(redemptionIntentHash, unlockSecret)}]{OstDotCom}{OpenSTUtility}
  ...
  \end{umlcall}
	...
	\begin{umlcall}[dt= 20, op={: processUnstaking(redemptionIntentHash, unlockSecret)}]{OstDotCom}{OpenSTValue}
  ...
  %\end{umlcall}
    	...
    	%revert unstaking could be initiatied from the redeemer side
	% beneficiary calls on processUnstaking with redempition intent hash
	\begin{umlcall}[dt= 250, op={: revertUnstaking(redemptionIntentHash)}]{Redeemer}{OpenSTValue}
	% require that the hashlock is the same
