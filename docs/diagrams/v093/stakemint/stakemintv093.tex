...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   % \end{umlcall}
    ...
    \begin{umlcall}[dt=15, op={: proveOpenST(blockHeightUC, rlpEncodedAccount, rlpParentNodes)}, return={emit OpenSTProven(blockHeightUC, storageRootUC, hashedAccount)}]{Facilitator}{CoreUC}
	...
	% check stake nonces of staker is less than staker nonce      
     ...
     % check calculated stakingIntentHash is equal to provided stakingIntentHash
      \begin{umlcallself}[op={require(merkleVerificationOfStake(stakerNonce, StakingIntentHash, rlpParentNodes, storageRoot)}]{OpenSTUtility}
      \end{umlcallself}       
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=1.5]
      ...
      \begin{umlfragment}[type=opt, name=bounty, label={ensure completion}, inner xsep=15]
       ...
       % \begin{umlcallself}[op={require(msg.sender = worker)}]{Branded Token Gateway}
        ...
        \end{umlcall}
      \end{umlfragment}
      \umlnote[x=12, y=-72, width=150]{bounty}{If stake was left unprocessed, unlock secret is known through mint. Bounty is always transferred to msg.sender of processStaking()}
...
\begin{tikzpicture}
  ...
  \end{umlcall}
    ...
    \begin{umlcall}[dt=73, op={: processRedeeming(redemptionIntentHash, unlockSecret)}]{Redeemer}{OpenSTUtility}
