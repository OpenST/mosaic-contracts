...
\begin{tikzpicture}
  \begin{umlseqdiag}
   ...
   %%%
    ...
    \begin{umlcall}[op={: redeem(uuid, amountUT, beneficiary, hashLock)}]{Redeemer}{OpenSTUtility}
    % transferfrom is called from openstutility on branded token
   \begin{umlcall}[dt=9,fill=green!20, op={: transferFrom(redeemer, OpenSTUtility, amountUT)}]{OpenSTUtility}{Branded Token}
   % ut is taken from redeemer to branded token
   \begin{umlcall}[dt= 12,fill=green!20, type=return, op={<<UT(amountUT)>>}]{Redeemer}{Branded Token}
   ...
   % event is emitted with redemptionintentHash which the beneficiary can listen to
    \begin{umlcall}[type=return, op={emit RedemptionIntentDeclared(redemptionIntentHash, brandedToken, beneficiary, amountUT, unlockHeight)}]{OpenSTUtility}{Beneficiary}
  \end{umlcall}
  \end{umlcall}
   \end{umlcall}   
    \end{umlcall}
    ...
    \begin{umlcall}[dt= 16, op={: confirmRedemptionIntent(uuid, redeemer, redeemerNonce, beneficiary, amountUT, unlockHeight, hashLock)}]{Beneficiary}{OpenSTValue}
	...
	\end{umlcallself}    
      % store the unstakes in the unstakes strcut with the redemptionhash as key 
     \begin{umlcallself}[op={store unstakes[redemptionIntentHash]}]{OpenSTValue}
	\end{umlcallself}     
     %emit the remeption Intent confirmed event 
     \begin{umlcall}[op={: RedemptionIntentConfirmed(redemptionIntentHash, redeemer, beneificary, amountST, amountUT, expirationHeight )}]{OpenSTValue}{Redeemer}
  \end{umlcall}
  \end{umlcall}

	%process redeem should begin now on UC side
	% redeemer calls on process redeeming on openstutility 
    \begin{umlcall}[dt= 19,op={: processRedeeming(redemptionIntentHash, unlockSecret)}]{Redeemer}{OpenSTUtility}
    % require hashlock is euqal to hash of unlock secret
    ...
    \begin{umlcall}[op={emit ProcessedRedemption(uuid, brandedToken, redeemer, beneficiary, amountUT, unlockSecret)}]{OpenSTUtility}{Beneficiary} 
  ...
  \end{umlcall}
  	...
  	%process unstaking should now begin on VC side
	% beneficiary calls on processUnstaking with redempition intent hash
	\begin{umlcall}[dt= 22, op={: processUnstaking(redemptionIntentHash, unlockSecret)}]{Beneficiary}{OpenSTValue}
	% require that the hashlock is the same
	\begin{umlcallself}[op={require hashLock = H(unlockSecret)}]{OpenSTValue}
	\end{umlcallself}
