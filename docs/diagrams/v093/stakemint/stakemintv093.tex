   ...
   % \end{umlcall}
    ...
    \begin{umlfragment}[type=alt, label=proceed, name=phasetwo, inner xsep=1.5]
      \begin{umlcall}[dt=6, fill=green!20, op={: processStaking(StakingIntentHash, unlockSecret)}]{Facilitator}{Branded Token Gate}
        ...
        \begin{umlcall}[dt=4, op={: processStaking(StakingIntentHash, unlockSecret)}]{Branded Token Gate}{OpenSTValue}
          ...
          \begin{umlcall}[fill=green!20, op={: transfer(simpleStake, amountST)}]{OpenSTValue}{OST}
            ...
            \end{umlcall}
          \end{umlcall}
          \begin{umlcall}[dt=5, type=return, op={emit ProcessedStake(uuid, StakingIntentHash, simpleStake, staker, amountST, amountUT, unlockSecret)}]{OpenSTValue}{Hunter}
          \end{umlcall}
        \end{umlcall}
      ...
      \begin{umlcall}[dt=5, op={: claim(beneficiary)}]{Facilitator}{Branded Token}
        \begin{umlcall}[fill=green!20, type=return, op={<<UT(amount)>>}]{Branded Token}{Beneficiary}
        \end{umlcall}
      \end{umlcall}
          ...
          \begin{umlcall}[dt=4, op={: processStaking(StakingIntentHash, unlockSecret)}]{Branded Token Gate}{OpenSTValue}
           ...
           % \begin{umlcallself}[op={require(msg.sender = gate = stakingAccount || stakingAccount = 0x)}]{OpenSTValue}
            ...
            \begin{umlcall}[fill=green!20, op={: transfer(simpleStake, amountST)}]{OpenSTValue}{OST}
              ...
              \end{umlcall}
            \end{umlcall}
            \begin{umlcall}[dt=5, type=return, op={emit ProcessedStake(uuid, StakingIntentHash, simpleStake, staker, amountST, amountUT, unlockSecret)}]{OpenSTValue}{Facilitator}
            \end{umlcall}
          \end{umlcall}
          % transfer bounty from gate to facilitator
      ...
      \begin{umlcall}[op={: revertMinting(StakingIntentHash)}]{Facilitator}{OpenSTUtility}
        % require mint not yet expired
        \begin{umlcallself}[op={require(expirationHeight <= block.number)}]{OpenSTUtility}
          \begin{umlcall}[dt=5, type=return, op={emit RevertedMint(uuid, StakingIntentHash, staker, beneficiary, amountUT)}]{OpenSTUtility}{Facilitator}
          \end{umlcall}
        \end{umlcallself}
      \end{umlcall}
    \end{umlfragment}
    ...
    \umlnote[x=40, y=-100, width=120]{phasetwo}{Only a facilitator can call revertStaking and then bounty is returned to the facilitator}
