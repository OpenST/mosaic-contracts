...
class ProgressRevertRedeemAssertion {
  ...
  async captureBalances(redeemer) {
    ...
    return {
      baseToken: {
        burner: await this._getBaseTokenBalance(burner),
        cogateway: await this._getBaseTokenBalance(this.cogateway.address),
        redeemer: await this._getBaseTokenBalance(redeemer),
      },
      token: {
  ...
  /**
   * Returns Base token balance (ETH) wrapped in BN.
   * @param {string} address Address for which balance is requested.
   * @return {Promise<BN>} Base token (ETH) Balance.
   * @private
   */
  async _getBaseTokenBalance(address) {
    return new BN(await this.web3.eth.getBalance(address));
  }
}
